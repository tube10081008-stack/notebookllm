...
## 9. v3 — 헌장·스카우트·수집함 (지식 공급 파이프라인)

> 진단: "소스가 적으면 어떤 시스템도 반복된다. 그러나 반복의 원인은 소스의 개수가 아니라
> **정보 다양성**이다." 양적 자동 수집은 반복을 오히려 강화한다 (서로를 재탕하는 소스 100개).
> 답은 자동 '수집'이 아니라 자동 '제안' — 필터가 가치다.

### 지식 헌장 (charter.json — canonical)
스테이션 생성 설문(목적/토픽/제외어/신뢰 소스/제안 상한)이 헌장이 된다. 헌장은 세 곳에 작용한다:
수집(Scout의 방향), 학습(거절 사유의 append-only 누적 = P-Reinforce policy 루프),
방향(추후 파인튜닝 데이터셋의 "전문 분야 비율" 기준선).

### Scout (자동 제안 파이프라인)
```
charter.feeds (공급 방향) + 최근 gaps (수요 신호)
  → RSS/Atom 수집 (의존성 없는 파서, SSRF 가드)
  → 제외어 게이트 → 토픽·결핍 키워드 게이트
  → 신규성 게이트: 기존 벡터와 유사도 > 0.85면 "이미 아는 것" — 제안 안 함
  → max_proposals개만 수집함(Inbox)에 제안
```
- **Scout는 지식 베이스에 아무것도 넣지 않는다** (불변식 5의 확장). ingest는 승인 후에만.
- **결핍 주도(gap-driven) 수집**: 모든 답변의 구조화 출력 `gaps`가 gaps.jsonl에 쌓이고,
  Scout가 이를 수요 신호로 사용한다. "토픽이 넓게 다루는 것"이 아니라
  "내 질문들이 드러낸 결핍"을 채우러 간다. 결핍 매칭은 토픽 매칭의 2배 가중.
- 거절 사유는 헌장 learned에 학습된다 — 과거를 고치지 않고 미래 행동을 바꾼다.

### 관련도 하한선 (일반 지식 혼합 모드)
v1의 "빈약한 소스로 억지 답변" 문제의 답. 코사인 검색은 무조건 뭔가를 반환하므로,
`RELEVANCE_FLOOR`(기본 0.45, 가설 수치)를 넘는 근거가 없으면 노트 강제 모드를 풀고
일반 지식과 혼합하되 노트 기반 부분만 인용 표시하게 한다. 응답에 `blended` 플래그,
UI에 "🌐 일반 지식 혼합" 배지. 하한선 미달 노트는 컨텍스트에 "관련도 낮음" 라벨이 붙는다.

## 10. 하이브리드 GitHub 영속 모드 & 에이전트 합성

### 저장 백엔드 추상화의 첫 실전 (§1-③의 회수)
`src/storage/`의 8개 프리미티브(readJSON/atomicWriteJSON/appendJSONL/...)를
GitHub Contents API 위에 재구현한 `github.js` 백엔드 추가. workspace.js의 도메인 로직은 무수정.

```
canonical (노트·원문·그래프·헌장·수집함·대화·이벤트·gaps)
   → GitHub private 저장소에 커밋 (지식 변경 = 커밋 = 감사 로그의 완성형)
derived (vectors.json)
   → 항상 로컬 fs (/tmp) — 크고, 재구축 가능하므로 커밋 히스토리를 오염시키지 않는다
콜드 스타트 → ensureVectorStore()가 notes에서 벡터를 그 자리에서 재구축 (불변식 2의 실전 효용)
```

- 활성화: `KNOWLEDGE_REPO` + `KNOWLEDGE_REPO_TOKEN` (반드시 private 저장소)
- 쓰기 충돌(409/422)은 sha 재조회 후 1회 재시도. 웜 인스턴스 캐시로 API 호출 절감.
- 성능 전제: 미리보기~경량 사용. 대량 축적의 본진은 여전히 로컬+git이다.

### 에이전트 = 헌장의 파생물 (기존 [에이전트 선택]의 폐기 근거)
고정 프리셋 6종 선택은 헌장과 **중복 입력**이었다 — 목적을 설문에 쓰고 또 페르소나를
고르는 것은 같은 정보를 두 번 말하는 것이고, 요리 스테이션과 ML 스테이션이 똑같은
'루나'를 받는다면 페르소나는 장식이다. 새 규칙:

- **정체성**(이름·이모지·말투·전문성·시스템 프롬프트)은 헌장에서 LLM이 합성 (`agentsmith.js`)
- **행동 파라미터**(증류·검색·답변 전략)는 LLM이 창작하지 못한다 — 검증된 6개
  아키타입(AGENT_BEHAVIORS) 중 하나로 "분류"만 시킨다 (eval로 지킨 경계 유지)
- **재조율(retune)**: 헌장 변경·지식 분포·거절 사유가 쌓이면 에이전트를 다시 파생시킨다
- 합성 실패·설문 생략 시 기존 프리셋으로 폴백 — 프리셋은 삭제가 아니라 안전망으로 강등

## 11. Reflection Rule (로드맵 전환 전 자문)
